...
namespace amc13{
  ...
  Launcher::~Launcher()
  {
    //Clean up all AMC13 objects
    while(AMCModule.size() > 0) {
      //Delete the last AMC13Module
      if(AMCModule.back() != NULL) {
	delete AMCModule.back();
      }
      AMCModule.pop_back();
    }
  }
					     ...
					     std::string const & currentToken,int state) 
{
  ...
  Module* Launcher::defaultModule() {  
    if(defaultAMC13no >= AMCModule.size()) {
      amc13::Exception::BadAMC13 e;
      e.Append("Tried to access bad AMC13 number\n");
      throw e;
    }  
    return AMCModule[defaultAMC13no];
  }
  

  AMC13* Launcher::defaultAMC13() {       
    return defaultModule()->amc13;
  }
