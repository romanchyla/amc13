...
namespace amc13{
  ...
  const char * Exception::exBase::StackTrace() const throw() 
  {
    if(stackBuffer == NULL)
      return stackError;
    return stackBuffer;
  ...
  void Exception::exBase::AppendStackLine(const char * line) throw()
  {
    //Check for NULL line
    if(line == NULL)
      return;
  ...
  void Exception::exBase::GenerateStackTrace() throw()
  {
